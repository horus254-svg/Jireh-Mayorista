...
async function checkoutWhatsapp(){
    ...
    // POST con el carrito en el cuerpo, no GET con todo en la URL —
    // con varios productos (sobre todo con nombres largos), la URL de
    // un GET puede crecer a varios miles de caracteres, y algunos
    // navegadores (Safari es más estricto que Chrome en esto) o
    // intermediarios de red pueden truncar o rechazar la petición sin
    // ningún aviso claro — eso explica el "Error al enviar el pedido"
    // o el "queda cargando" que pasa más seguido en Safari. El backend
    // (doPost en code.gs) ya soporta este mismo formato sin cambios.
    const cuerpoPedido = {
    ...
    try{
        ...
        // Sin esto, si Apps Script tarda demasiado en responder (puede
        // pasar ocasionalmente, sobre todo si el servidor de Google
        // estaba "frío"), el fetch queda esperando indefinidamente y el
        // botón se ve trabado en "Enviando pedido..." para siempre, sin
        // ningún error que el catch pueda atrapar — esto le pone un
        // límite de tiempo real a la espera.
...
`;
        ...
        if(error.name === "AbortError"){
            // El servidor estaba tardando demasiado en responder — no
            // hay forma de saber desde acá si el pedido se llegó a
            // guardar o no, así que se le avisa al cliente con
            // claridad para que no haga un pedido duplicado por las
            // dudas, en vez de mostrar un error genérico de conexión.
            mostrarToast("La confirmación está demorando más de lo normal. Si no recibiste la respuesta por WhatsApp en unos minutos, contactanos antes de volver a enviar el pedido.", "error");
        }else{
            mostrarToast("Error al registrar el pedido. Intentá de nuevo.", "error");
        }
